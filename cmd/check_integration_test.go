...
func TestCheckWithFakeRealPipeline_StillReturnsStableText(t *testing.T) {
	...
	for _, token := range []string{"Cluster:", "Milvus Version: 2.6.1", "Arch Profile: v2.6", "Summary: databases=1 collections=1 total_rows=123 pods=1", "K8s Summary: ready=1 not_ready=0 services=1 endpoints=1 resource_usage_source=auto resource_usage=available (1/1 pods have metrics)", "Databases: default(book)"} {
